...
const isBackNoiseSegment = (segment) => {
  ...
  if (/^[\d\s-]+$/.test(value) && value.replace(/[\s-]/g, "").length >= 8) {
    return true;
  }
...
const stripIdNumbers = (text) =>
  text
    .replace(/\bvid\s*[:\-]?\s*(?:\d[\s-]?){12,16}/gi, " ")
    .replace(/(?:\d[\s-]?){12,}/g, " ")
    .replace(/\s{2,}/g, " ")
    .trim();
...
export const parseBack = (rawText) => {
  ...
  let address = stripIdNumbers(segments.join(", ").replace(/\s+/g, " ").trim());
